...
function ActivityCalendarCard({ matches, puuid, platform }: { matches: RecentMatchSummary[]; puuid: string; platform: string }) {
  ...
  // hydration-date gotcha the project guide already documents elsewhere) — grid
  ...
  // hydration gotcha around Intl without a real locale (see the project guide).
            ...
            <div key={pool.position} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "flex", height: 6, width: "100%", overflow: "hidden", borderRadius: 999, background: `${COLORS.muted}26` }}>
                {pool.champions.map((c, i) => (
                  <div
                    key={c.championName}
                    style={{ width: `${(c.games / pool.totalGames) * 100}%`, background: i === 0 && isOneTrick ? COLORS.gold : `${COLORS.rose}80` }}
                  />
                ))}
              </div>
